...
def get_args():
    ...
    # 模型参数
    parser.add_argument('--model_type',nargs='?',default='mybasemodel')

    parser.add_argument('--emb_dim',type=int,default=16)
    parser.add_argument('--dnn_layer',nargs='?',default='[128,128,64,32]')
    parser.add_argument('--reg_1', type=float, default=1e-6)
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--batch_size', type=int, default=512)
    parser.add_argument('--word_size', type=int, default=1000)
    parser.add_argument('--filter_num', type=int, default=8)
    parser.add_argument('--filter_size',nargs='?',default='[2,3,4]')
...
def main():
    ...
    train,val=make_train_val(df,day=14)
    ...
    text_feature_dict={'description':100}
    ...
    for epoch in range(epochs):
        ...
        for idx in range(n_batch):
            ...
            values_dict=convert_feed_dict(batch_data,sparse_features+dense_features,action)
            feed_dict=model._get_feed_dict(values_dict)
        ...
        task_loss_log.append(task_loss)
        print('epoch:{},loss:{},task_loss:{}'.format(epoch+1,loss,task_loss))

        # 预测验证集 输出auc
        val_predict_ans=predict_by_batch(model,sess,val,args.batch_size*4,
                                     sparse_features+dense_features,action)
