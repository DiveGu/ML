...
dense_features = ['videoplayseconds']+['userid_5day_{}_sum'.format(b) for b in ACTION_LIST]+\
['feedid_5day_{}_sum'.format(b) for b in ACTION_LIST]+\
['userid_5day_{}_mean'.format(b) for b in ACTION_LIST]+\
['feedid_5day_{}_mean'.format(b) for b in ACTION_LIST]
...
text_features=['des_words','manual_tag','machine_tag','manual_keywords','machine_keywords','user_watch_history']
...
def convert_feed_dict(df,cols,y):
    ...
    for c in cols:
        ...
        if c in ['des_words','ocr_words','asr_words',\
                 ...
                 "user_click_avatar_history",  "user_forward_history",'user_watch_history']:
...
def generate_sample(df,action,day=14):
    ...
    for stat_cols in [
        ...
        for target_day in range(2, 15 + 1):
            feats=[]
...
#     df_neg = df_neg[:sample_neg_num]
    ...
    feats_final=list(set(feats_final))
    col = ["userid", "feedid", "date_", "device"]+feats_final + ACTION_LIST
    print(col)
...
def test_concat(df_test):
    df=pd.read_csv(ROOT_PATH+'/wechat_algo_data1/user_history_list.csv')
#     df = df.drop_duplicates(subset=['userid', 'feedid'], keep='last')
    ...
    for stat_cols in [
        ...
        f = '_'.join(stat_cols)
        stat_df = pd.DataFrame()
        for target_day in range(14, 15 + 1):
            feats=[]
            left, right = max(target_day - n_day, 1), target_day - 1
            tmp = df[((df['date_'] >= left) & (df['date_'] <= right))].reset_index(drop=True)
            tmp['date_'] = target_day
    ...
    sample = sample.join(df, on=["userid",'date_'], how="left", rsuffix="_static")
    ...
    return sample[features+list(set(feats_final))]
...
def make_train_val(df,day):
    ...
    day=min(day,14)
    val=df[df['date_']==day]
    val=val.drop_duplicates(subset=['userid', 'feedid'], keep='last')
...
def main():
    ...
    all_text_feature_dict={
                            ...
                            'machine_keywords':5,
                            'user_like_history':5,
                            "user_read_comment_history":5, 
                            "user_click_avatar_history":5,
                            "user_forward_history":5,
                            'user_watch_history':20,
                           }
...
#         cur_text_features=text_features+['user_{}_history'.format(action)]
        if(action in ["read_comment1"]):
            cur_text_features=text_features+['user_watch_history']
        else:
            cur_text_features=text_features
        print(cur_text_features)
