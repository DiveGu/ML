...
for f in tqdm(['userid', 'feedid', 'authorid','feed_cluter','video_time_group',]):
    df[f + '_count'] = df[f].map(df[df['date_']<16][f].value_counts())
...
# df['videoplayseconds_in_tag_mean'] = df.groupby('one_tag')['videoplayseconds'].transform('mean')
df['videoplayseconds_in_feed_cluter_mean'] = df.groupby('feed_cluter')['videoplayseconds'].transform('mean')
...
trn_x = train[(train['date_'] < 14)&(train['date_'] >= 1)].reset_index(drop=True)
...
r_dict = dict(zip(y_list[:4], [150,180,10,10]))
for y in y_list[:4]:
    print('=========', y, '=========')
    t = time.time()
    clf = LGBMClassifier(
...
#     SUB_PATH+'/b_tree_%.6f_%.6f_%.6f_%.6f_%.6f.csv' % (weighted_uauc, uauc_list[0], uauc_list[1], uauc_list[2], uauc_list[3]),
#     index=False
# )

test[['userid', 'feedid'] + y_list[:4]].to_csv(
    SUB_PATH+'/b_tree_2.csv',
    index=False
)