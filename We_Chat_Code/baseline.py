...
flags.DEFINE_string('model_checkpoint_dir', '/testcbd017_gujinfang/GJFCode/WeChat_2021/Code/data/model', 'model dir')
flags.DEFINE_string('root_path', '/testcbd017_gujinfang/GJFCode/WeChat_2021/Code/data/', 'data dir')
...
def main(argv):
    ...
    if stage in ["evaluate", "offline_train", "online_train","submit"]:
        # 计算所有行为的加权uAUC
        print(eval_dict)
    ...
    if stage == "submit":
        print('Average prediction time of 2000 samples with different target behaviors (MS)')
        print(predict_time_cost)
        print('Average prediction time of 2000 samples of single target behavior (MS)')
